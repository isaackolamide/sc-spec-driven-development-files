...
describe('API Routes Integration Tests', () => {
    ...
    it('should successfully schedule a new appointment on POST /api/appointments', async () => {
        ...
        const ailments = await (await app.request('/api/ailments')).json();
        const therapies = await (await app.request('/api/therapies')).json();
        const apptPayload = {
            agentId: agents[0].id,
    ...
    it('should successfully update status on PATCH /api/appointments/:id', async () => {
        ...
        const appointments = await (await app.request('/api/appointments')).json();
